...
class InMemoryProviderCredentialVault:
    ...
    def upsert(self, tenant_id: str, provider: str, route_id: str, payload: CredentialUpsertRequest, actor: str, trace_id: str) -> ProviderCredentialData:
        del trace_id
        ...
        verification = dict(self.verifier.verify(tenant_id=tenant_id, provider=provider, route_id=route_id, secret=secret))
        now = utc_now()
        ...
        return _record_data(record, label, keyring_ready=True)

    def revoke(self, tenant_id: str, provider: str, route_id: str, payload: CredentialRevokeRequest, actor: str, trace_id: str) -> ProviderCredentialData:
        del trace_id
        label, _ = _route(provider, route_id, self.env)
        now = utc_now()
        with self._lock:
        ...
        return _record_data(current, label, keyring_ready=self.keyring is not None)
...
class MySQLProviderCredentialVault(InMemoryProviderCredentialVault):
    def __init__(self, database_url: str, keyring: CredentialKeyring | None, *, verifier: CredentialVerifier | None = None, env: Mapping[str, str] | None = None) -> None:
        super().__init__(keyring, verifier=verifier, env=env)
    ...
    def upsert(self, tenant_id: str, provider: str, route_id: str, payload: CredentialUpsertRequest, actor: str, trace_id: str) -> ProviderCredentialData:
        ...
        secret = payload.secret.get_secret_value()
        verification = dict(self.verifier.verify(tenant_id=tenant_id, provider=provider, route_id=route_id, secret=secret))
        now = utc_now(); version = payload.expected_version + 1; credential_id = f"provider_credential_{uuid4().hex[:16]}"
        envelope = self.keyring.encrypt(tenant_id=tenant_id, provider=provider, route_id=route_id, credential_id=credential_id, credential_version=version, plaintext=secret)
        request_sha256 = canonical_sha256({"provider": provider, "route_id": route_id, "version": version, "fingerprint": envelope.secret_fingerprint, "reason": payload.reason, "verification": verification})
        with self.engine.begin() as conn:
            ...
            if locked:
                ...
                if was_active:
                    ...
                    conn.execute(
                        text("UPDATE airank_provider_credentials SET is_current=0 WHERE id=:id"),
                        {"id": rotated_from_id},
                    )
        ...
        return _record_data(self._from_row(row, latest_hash), label, keyring_ready=True)

    def revoke(self, tenant_id: str, provider: str, route_id: str, payload: CredentialRevokeRequest, actor: str, trace_id: str) -> ProviderCredentialData:
        label, _ = _route(provider, route_id, self.env); now = utc_now()
        with self.engine.begin() as conn:
        ...
        return _record_data(self._from_row(updated, latest_hash), label, keyring_ready=self.keyring is not None)
...
def raise_vault_error(exc: CredentialVaultError) -> None:
    if exc.code in {"PROVIDER_NOT_SUPPORTED", "PROVIDER_ROUTE_NOT_FOUND", "CREDENTIAL_NOT_FOUND"}:
        status = 404
    elif exc.code in {"STATE_VERSION_CONFLICT", "CREDENTIAL_UNCHANGED", "CREDENTIAL_ALREADY_REVOKED"}:
        status = 409
    elif exc.code in {
...
def upsert_provider_credential(provider: str, route_id: str, payload: CredentialUpsertRequest, tenant_id: str = Header(default="tenant_demo", alias="tenant-id"), trace_id: Optional[str] = Header(default=None, alias="X-AIRank-Trace-Id"), authenticated_actor: Optional[str] = Header(default=None, alias="X-AIRank-User-Id"), permission_header: Optional[str] = Header(default=None, alias="X-AIRank-Permissions")) -> ProviderCredentialResponse:
    ...
    try:
        data = get_provider_credential_vault().upsert(tenant_id, provider, route_id, payload, actor, trusted_trace)
    except CredentialVaultError as exc:
        raise_vault_error(exc)
...
def revoke_provider_credential(provider: str, route_id: str, payload: CredentialRevokeRequest, tenant_id: str = Header(default="tenant_demo", alias="tenant-id"), trace_id: Optional[str] = Header(default=None, alias="X-AIRank-Trace-Id"), authenticated_actor: Optional[str] = Header(default=None, alias="X-AIRank-User-Id"), permission_header: Optional[str] = Header(default=None, alias="X-AIRank-Permissions")) -> ProviderCredentialResponse:
    require_provider_admin(permission_header); actor = trusted_actor(authenticated_actor); trusted_trace = trace_id or f"trc_{uuid4().hex[:16]}"
    try:
        data = get_provider_credential_vault().revoke(tenant_id, provider, route_id, payload, actor, trusted_trace)
    except CredentialVaultError as exc:
        raise_vault_error(exc)
    return ProviderCredentialResponse(data=data, meta=response_meta(trusted_trace))
