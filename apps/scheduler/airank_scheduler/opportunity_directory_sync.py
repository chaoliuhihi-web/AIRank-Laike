...
class OpportunityDirectorySyncQueuePreview:
...
@dataclass(frozen=True)
class OpportunityDirectorySyncDispatchRecord:
    binding_id: str
    team_id: str
    job_id: str
...
class MySQLOpportunityDirectorySyncScheduler:
    """Queues governed action-team directory jobs without persisting credentials."""
    ...
    def __init__(
        ...
        *,
        ...
        scheduler_id: str = "airank-opportunity-directory-sync-scheduler",
    ) -> None:
    ...
    def preview(
        ...
        return OpportunityDirectorySyncQueuePreview(
    ...
    ) -> list[OpportunityDirectorySyncDispatchRecord]:
        if limit < 1:
            raise ValueError("dispatch limit must be positive")
        dispatched_at = now or utc_now()
        records: list[OpportunityDirectorySyncDispatchRecord] = []
    ...
    def _dispatch_one(
    ...
    ) -> OpportunityDirectorySyncDispatchRecord | None:
        scope_sql, params = self._scope_sql("binding")
        params["now"] = dispatched_at
        lock_sql = (
            " FOR UPDATE SKIP LOCKED" if self.engine.dialect.name == "mysql" else ""
        )
        with self.engine.begin() as conn:
            ...
            job_id = _stable_id("job_odir", tenant_id, binding_id, due_marker)
            ...
            conn.execute(
                text(
                    ...
                    INSERT INTO airank_async_jobs (
                      ...
                      'opportunity.directory.sync', 'queued', 45,
            ...
            conn.execute(
                text(
                    ...
                    INSERT INTO airank_audit_events (
                      ...
                      'opportunity_action_team_sync_binding', :entity_id,
                      :payload_json, :created_at
                    )
                    """
                ),
                {
                    "id": _stable_id("audit_odir", tenant_id, job_id),
                    "tenant_id": tenant_id,
                    "project_id": project_id,
                    "actor": self.scheduler_id,
        ...
        return OpportunityDirectorySyncDispatchRecord(
            binding_id=binding_id,
            team_id=team_id,
            job_id=job_id,
            scheduled_at=dispatched_at,
