...
13. `geo-citation-lab` 的来源字典已改造为 AIRank 自有 Source Registry：只登记项目 Citation 中实际出现的精确 DNS host，未知来源保持 `unclassified`；人工复核追加记录一级/细分类、主体、置信度、权威度、用途、风险、证据 URL/说明、有效期和 supersedes 链。旧版本、原始 Citation 与样本不被覆盖，幂等回放旧请求也不会把旧版本显示成当前分类；公开 CN-GEO 字典的版本化批量导入、双人复核和报告包联动仍未完成，因此状态保持 `partial`。
...
| geo-citation-lab | 来源类型与权威度治理 | 支撑来源结构、缺口和人工复核 | `data/reference/source_types.csv` | 域名 → 类型/状态/置信度/证据 | 参考表和人工审核 | CC-BY-4.0 | 已实现项目级精确 host 注册表；只读取 Citation 实际出现域名，未知保持 `unclassified`；追加式人工复核保存分类、主体、置信度、权威度、用途、风险、依据、有效期、审核人、supersedes、请求 hash 与审计事件；MySQL、API 和证据中心已贯通 | 公开字典的版本化批量导入、双人复核 benchmark、分类过期运营队列和客户报告包联动仍缺 | absorb | Source Registry | P1 | partial | 真实 MySQL 验证未分类、v1/v2、旧版幂等回放和审计；隔离浏览器完成未分类到 v2，桌面/390px 无页面级溢出且 console 0 warning/error，随后清理 QA 数据 |
...
| yao-open-tools | TokDoc 报告与版本快照 | 客户报告归档与公开交付 | `tools/TokDoc` | HTML/PDF/Word → 版本/链接 | 本地存储 | MIT | 已有内容寻址 JSON 客户证据包、对象 hash、本地下载校验和 packet 级回执；v2 增加不含原始回答正文的事实声明/裁决边界与证据 hash 索引，历史 v1 可读取 | HTML/PDF/Word 渲染、签名和公开验证工具仍缺 | reference_only | Report Artifact Store | P2 | partial | 真实 MySQL 报告生成证据包并重算事实覆盖/准确率；下载前复验 SHA-256，回执绑定 packet/hash；未通过门禁时阻断 |
...
- Source Registry 只聚合 Citation 表里实际存在的精确 host，不按父域或品牌名自动猜测。`20260808_0020` 在真实 MySQL 建立追加式分类修订表；API 验证未分类、人工 v1/v2、乐观锁、可信操作者、幂等旧版回放与审计。隔离浏览器租户完成从 `unclassified` 到 v2 的真实点击闭环，桌面与 390px 无页面级横向溢出且 console 为 0 warning / 0 error；14 行 QA 数据随后精确清理为 0。
