...
10. `Customer Evidence Packet` 已升级为 AIRank 自有 `airank.report-evidence-packet.v3`：仅在基线/复测质量门禁均通过后生成，内容寻址保存公式、限制、风险、样本/引用/事实准确性/来源治理/对象索引与哈希；前端下载后本地复验 SHA-256，下载回执必须绑定 packet/hash。来源治理按 Citation 的精确 host 写入当前分类修订与有效性，不复制人工说明正文；未分类、过期、未知权威、禁止用途和无法解析 host 均进入限制项，覆盖不完整时禁止整体权威性结论。历史 v1/v2 包仍可读取但不会伪装成 v3；HTML/PDF/Word 渲染、签名和正式空白评分表仍未实现，因此状态保持 `partial`。
11. 品牌/竞品事实准确率不再是占位指标：回答中的事实声明按精确字符边界、声明类型和主体登记，追加式人工裁决只能绑定当前有效、人工审核、可披露、无开放冲突且能落到原文精确边界的 `FactRevision`。事实或来源过期、被替代或发生冲突时，旧裁决保留但自动退出当前商业指标；`insufficient` 单独统计且不绑定虚构事实。控制台可下钻登记和裁决，报告与 v3 证据包按当前证据重新计算覆盖率和准确率。
...
13. `geo-citation-lab` 的来源字典已改造为 AIRank 自有 Source Registry：只登记项目 Citation 中实际出现的精确 DNS host，未知来源保持 `unclassified`；人工复核追加记录一级/细分类、主体、置信度、权威度、用途、风险、证据 URL/说明、有效期和 supersedes 链。旧版本、原始 Citation 与样本不被覆盖，幂等回放旧请求也不会把旧版本显示成当前分类。Source Registry 已进入 v3 客户证据包和报告下载提示；公开 CN-GEO 字典的版本化批量导入、双人复核 benchmark 和到期运营队列仍未完成，因此状态保持 `partial`。
14. `yao-geo-page-blueprint` 已改造为 AIRank 自有 `intervention.page-blueprint@1.1.0`：只接受当前已批准、可生成、无开放冲突且具有 source hash 与精确原文边界的事实；缺证据时只返回 `needs_evidence`，不生成补写文案。FAQ、事实页、产品页、案例页、研究页、比较页、JSON-LD 和 `llms.txt` 共用版本化 Claim/FactRevision/ClaimSupport 绑定，请求标题与编辑方向只进入 brief hash，公开标题由已审核事实元数据确定性生成。来源正文与登记 hash 不一致、可执行嵌入内容、逐主张支持不完整或正文 hash 变化都会失败关闭；发布快照 v2 固化蓝图 hash 与 Skill 版本。专用 Explainer、同口径 Comparison、CMS 字段映射和完整 Schema.org 语义验证仍未完成，因此保持 `partial`。
...
| yao-geo-skills | `yao-geo-page-blueprint` | 将证据缺口转成页面结构 | `skills/yao-geo-page-blueprint` | 缺口/事实 → 模块、Schema、CMS 字段 | 已审核事实、页面诊断 | MIT | `intervention.page-blueprint@1.1.0` 已把 8 类页面产物绑定到已审核 FactRevision、ClaimSupport、source hash 和精确原文边界；缺证据时不生成正文；控制台使用真实 API 选择合格事实并展示蓝图 hash；审核与发布快照校验正文和蓝图完整性 | 页面审计缺口尚未自动编排；专用 Explainer/Comparison、CMS 字段映射与完整 Schema.org 语义验证仍缺 | adapt | Page Intervention Skill | P1 | partial | contract/holdout/adversarial 评测通过；真实 HTTP/MySQL 验证来源→事实→批准→FAQ 蓝图→审核→不可变导出，编辑方向明文未落库；后续补多类型 Schema benchmark |
...
| 来源仓库 | 能力名称 | 业务价值 | 代码位置 | 输入输出 | 依赖条件 | 许可证 | AIRank 当前能力 | 差距 | 吸收方式 | 目标模块 | 优先级 | 状态 | 验收方法 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| geo-citation-lab | 引用选择 vs 引用吸收 | 避免把“被列为来源”误当“支持了回答” | `01-geo-experiment-data-report/03-pipeline` | 回答/页面 → selection/absorption 特征 | 完整回答和页面正文 | 分范围许可 | 已新增不可变 Answer Claim、追加式 Citation Support Review、支持/矛盾/不足标签、独立支持率；品牌/竞品事实声明另走 Fact Accuracy Review，不把引用支持度混成事实准确率；统一 DNS-pinned 安全抓取会内容寻址保存原始页与正文、生成精确字符边界片段；只有人工审核同时绑定 completed capture、raw object、segment 和 exact excerpt 才进入可交付支持率；API、Worker、MySQL、证据中心和浏览器真实闭环已接入 | 双人复核队列、人工标注集与一致性 benchmark 仍缺 | absorb | Citation + ClaimSupport | P0 | partial | selection count、support rate、fact accuracy 分开展示；旧复核不覆盖；无合格来源页支持率为 null；真实 Example 页面对不相关长回答被人工判为 insufficient，支持率为 0% 而非伪造正值 |
...
| geo-citation-lab | 来源类型与权威度治理 | 支撑来源结构、缺口和人工复核 | `data/reference/source_types.csv` | 域名 → 类型/状态/置信度/证据 | 参考表和人工审核 | CC-BY-4.0 | 已实现项目级精确 host 注册表；只读取 Citation 实际出现域名，未知保持 `unclassified`；追加式人工复核保存分类、主体、置信度、权威度、用途、风险、依据、有效期、审核人、supersedes、请求 hash 与审计事件；MySQL、API、证据中心和 v3 客户证据包已贯通 | 公开字典的版本化批量导入、双人复核 benchmark 和分类过期运营队列仍缺 | absorb | Source Registry | P1 | partial | 真实 MySQL 验证未分类、v1/v2、旧版幂等回放、报告包历史版本和审计；隔离浏览器确认 1/2 来源有效时不生成整体权威性结论，桌面/390px 无页面级溢出且 console 0 warning/error，随后清理 QA 数据 |
...
| geo-citation-lab | 数据质量门禁 | 防止猜测缺失字段或误删样本 | `quality_report.json`、tests | 数据仓库 → checks/known limitations | 固定依赖与清单 | MIT code | `airank.measurement-quality.v4` 执行 24 项检查；每组问题/Provider/Cohort/采集面/模型至少 3 个独立 sample index 与 session；Web/App 还必须由采集器确认进入全新会话；每个任务样本加载独立 Evidence Manifest，失败/阻塞也必须有原始响应 hash；API、Web、App 和人工导入按采集面分别门禁；存在品牌/竞品事实声明时，事实覆盖率必须完整才允许形成事实准确率，未登记、未裁决或仅 `insufficient` 均进入限制项 | App 采集器、全库派生表重建、双人事实标注 benchmark 和多格式交付物视觉门禁仍缺 | adapt | Evidence data gate | P1 | partial | 失败批次可审计但不可发布；单次采样、重用会话或未验证全新 Consumer 会话必须 `quality_blocked`；质量报告具备 data/report SHA-256、分采集面汇总与 known limitations；失败快照、终端证据、事实覆盖或复测口径不合格时 `publishable=false` |
...
| 来源仓库 | 能力名称 | 业务价值 | 代码位置 | 输入输出 | 依赖条件 | 许可证 | AIRank 当前能力 | 差距 | 吸收方式 | 目标模块 | 优先级 | 状态 | 验收方法 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TokHub | Provider manifest | 统一模型、端点、能力和生命周期 | channel/store models | manifest → eligible upstreams | provider catalog | Apache-2.0 | 四平台 manifest、别名、能力、官方 host、模型生命周期和请求默认值契约已实现；`20260808_0022` 持久化 manifest 默认值与 route 实际请求契约，凭证轮换和参数变化追加版本，配置指纹可关联历史样本审计 | 后台编辑、模型目录自动同步与迁移审批仍缺 | adapt | Provider Manifest | P0 | partial | manifest/schema/迁移通过；凭证轮换保留 3 个不可变版本；四平台 12 条真实审计均匹配调用时指纹 |
...
| haidian | 离线评审包与空白评分表 | 把风险、假设、来源、指标、文件和正文放入一个可归档交付包 | `docs/review-packets.md`、`scripts/export_review_packet.py`、`scripts/generate_formal_scorecard.py` | 已通过门禁的产物 → manifest/HTML/PDF/评分材料 | 不可变 artifact、导出引擎、人工评审 | NOASSERTION；仅参考方法 | AIRank 自有 `airank.report-evidence-packet.v3` 已覆盖质量门禁、风险、假设、指标公式、限制、样本/引用/事实准确性/来源治理/对象索引和内容 hash；包进入不可变对象存储，下载端复验 SHA-256，回执绑定 packet/hash；历史 v1/v2 只读兼容 | HTML/PDF/Word 离线渲染、数字签名、公开验证工具和 eligibility 前置空白评分表仍缺 | reference_only | Customer Evidence Packet | P1 | partial | 质量阻断/旧报告/缺样本均禁止生成；真实 MySQL 报告可导出并复验 hash，正常未提及样本保留，事实与来源结论能落到精确边界/host/revision hash，来源治理不完整时禁止整体权威性结论 |
...
- 页面干预不再把请求标题或编辑方向直接拼成营销文案：`intervention.page-blueprint@1.1.0` 只编排已审核事实和精确来源边界，公开标题由已审核事实元数据确定性生成，并返回 sections、claim bindings、结构化数据、正文与内容寻址蓝图 hash。真实 HTTP/MySQL 隔离租户已跑通 FAQ 蓝图、可信审核人与 `airank.publish-snapshot.v2` 导出，验证两个 brief 字段明文均未落库、逐 Claim 支持和发布快照可追溯，验收后清理为 0 行。
