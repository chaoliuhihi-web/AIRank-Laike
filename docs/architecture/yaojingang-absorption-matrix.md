...
| 来源仓库 | 能力名称 | 业务价值 | 代码位置 | 输入输出 | 依赖条件 | 许可证 | AIRank 当前能力 | 差距 | 吸收方式 | 目标模块 | 优先级 | 状态 | 验收方法 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| geo-citation-lab | 引用选择 vs 引用吸收 | 避免把“被列为来源”误当“支持了回答” | `01-geo-experiment-data-report/03-pipeline` | 回答/页面 → selection/absorption 特征 | 完整回答和页面正文 | 分范围许可 | Provider 原生选择层使用 `airank.provider-native-citation.v2` 白名单解析并保存 native type/精确 JSON path/source id，任意 URL 不计 Citation；已新增不可变 Answer Claim、追加式 Citation Support Review、支持/矛盾/不足标签、独立支持率；品牌/竞品事实声明另走 Fact Accuracy Review；统一 DNS-pinned 安全抓取保存来源正文和精确边界；生产审核已接入盲态双人复核、分歧裁决、benchmark 隔离、kappa 门禁、证据中心和 v7 客户包 | 目前只有千问 Responses 路由完成真实原生引用重复采样；真实客户人工标注 benchmark 仍为 0/20，claim 细粒度框选体验仍可优化 | absorb | Citation + ClaimSupport | P0 | partial | 7/7 原生结构 benchmark；任意调试 URL 不计引用；selection count、support rate、fact accuracy 分开展示；不同账号双人一致或第三人裁决才可商业核验 |
