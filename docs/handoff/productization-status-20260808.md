...
- 最新严格上线门禁（Provider 原生引用代码基线 `ca18fe8`）：工作树、GitHub/Gitee `main` 同步、diff、运行产物、Python 3.11.15、Node 24.13.1、185 个 contract、75 个 acceptance、7 个 scheduler、35 个 worker、48 个 evidence、26 个 Provider Gateway、7/7 原生引用 benchmark、全部分包测试、10 个核心 Skill 30/30、Web 构建、真实 MySQL 28/2、离线 SQL与真实 Alembic `0026` 均为 `PASS`；全量测试另为 444 passed / 30 skipped。总状态仍为 `BLOCKED`：生产 API 强制认证/Yudao 未配置，生产 HTTPS S3/MinIO 未配置；在 `--require-optional-capabilities` 下，外部 Crawler/KB/内容/workflow/Hermes 仍为 `dev_only`；消费端 L3 真实生成仍为 `0/4`，千问/DeepSeek 为验证码阻塞，豆包/Kimi 为登录阻塞。千问原生来源选择、API 四平台重复采样、v2 源证据与派生状态巡检、v7 可核验客户 ZIP、本地发布证据和工程门禁的通过不替代 Citation Support、Consumer Web/App、真实客户 benchmark 或生产站点门禁。
