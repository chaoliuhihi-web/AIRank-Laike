...
- Feature commit `17060de` adds AIRank-owned persistent Operation Guard semantics to Provider credential upsert/revoke; follow-up commit `5e7da04` removes cross-tenant queue interference from a real integration fixture. Both commits are synchronized to GitHub/Gitee `main` and `codex/evidence-productization`.
...
- Contract tests cover key rotation-stable replay, payload conflict, failed L3 suppression, concurrent unknown outcomes and mandatory headers. Real MySQL verifies upsert/replay/rotate/revoke, three-event operation chains, linked credential events, secret/key absence and complete random-tenant cleanup.
- On clean, dual-remote-synchronized commit `5e7da04`, the strict executable gate passes 242 contract, 6 crawler-lite, 99 acceptance, 20 Scheduler, 45 Worker, 16 score, 48 evidence, 23 outbound-security, 32 Provider Gateway and 10 Xinghe adapter tests; citation benchmark is 7/7, Skills are 33/33, Node 24 Web build passes, real integration is `37 passed, 2 skipped`, and offline/real Alembic pass at `0041`.
