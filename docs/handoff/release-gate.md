...
- On clean, dual-remote-synchronized commit `d7edc1d`, the strict executable gate passes 261 contract, 6 crawler-lite, 110 acceptance, 20 Scheduler, 50 Worker, 16 score, 48 evidence, 23 outbound-security, 32 Provider Gateway, 10 Xinghe adapter, 7/7 citation and 33/33 Skill eval tests. Node 24 build, real integration `40 passed, 2 skipped`, offline SQL and real Alembic `0046`/111 AIRank tables pass; the wider default suite is `602 passed, 40 skipped` and npm production audit reports zero vulnerabilities. The overall result correctly remains `BLOCKED` on production auth/storage/Yudao, DeepSeek migration, Consumer Browser evidence and customer receipts.
- The corrected post-migration runtime uses `AIRANK_PROVIDER_MODE=api`; one fresh L3 probe for each configured route returned generation verified for Doubao, Qianwen, Kimi and DeepSeek. Earlier restart checks that fell back to Browser L2 were explicitly discarded. This confirms local Provider API execution only and does not replace Consumer Web/App evidence; the exposed Kimi credential still requires rotation before production.

## 2026-08-09 multi-format Customer Evidence Packet v8 gate

- Feature commit `24b02f9` upgrades new customer artifacts to `airank.report-evidence-packet.v8` and review bundle v2. The immutable ZIP now contains canonical manifest, printable HTML, fixed-Chromium A4 PDF, editable deterministic OOXML Word, blank formal scorecard, README and `SHA256SUMS`; historical v1–v7 packets remain read-only compatible.
- HTML/PDF/Word use one frozen manifest and cannot invent additional metrics or conclusions. PDF time metadata is normalized to the report generation time. Word uses A4 geometry, explicit Chinese/Latin typography, paragraph rhythm, heading hierarchy and a deliberate appendix page. A rendering failure is converted to `500 REPORT_RENDERING_FAILED` and no partial object is persisted.
- Offline verification still requires the API/download-receipt archive SHA-256. It deterministically rebuilds the manifest, README, HTML, Word and scorecard, verifies every member checksum, and checks the PDF structure and active-content boundary without launching Chromium. It does not claim that package self-consistency is a digital signature.
- Final artifacts were rendered and inspected page by page: PDF page two no longer clips the sample hash column or paints the page background; Word renders all Chinese glyphs, keeps evidence headings with content and starts the sample/source/hash appendix on page two. The document QA workflow materially changed the Word output from a structural OOXML placeholder into a customer-readable governed report.
- Repository regression is `641 passed, 40 skipped`; real MySQL integration is `40 passed, 2 skipped`; the 13-source/67-row/21-Skill absorption matrix, Node production build, bundle budget and npm high-severity audit pass. The v8 CLI independently verifies a seven-member packet using its external archive hash.
- Clean feature commit images were built with pinned Python 3.11/Node 22 bases. Backend runs as `10001:10001`, Web as `101`; backend Chromium generated a Chinese PDF inside the final image, Word generation passed, Web Nginx syntax passed with the Compose `api` host present, and both final images have zero fixed HIGH/CRITICAL findings under the CI-pinned Trivy scanner. SPDX JSON SBOM generation also succeeds for both images.
- Commercial status remains `BLOCKED / NO-GO`. The repository-local report deliverable is complete, but production Yudao identity/tenant binding, TLS MySQL, HTTPS versioned S3/MinIO plus authorized write/read probe, KMS/HSM-managed credential keys, exposed-credential rotation attestation, final public origin, exact publisher allowlists, DeepSeek replacement quota/migration, Consumer Web/App L3 evidence, real customer reviewer benchmark, real customer publish/notification receipts and elapsed T+7/T+14/T+30 observations remain external release gates. Digital signing and an independently hosted public verification page remain explicitly `partial` rather than being presented as complete.
