...
Generated: 2026-08-09T14:35:37+08:00

Verified feature commit: `d7edc1d`
...
| GitHub `main` and feature branch contain `d7edc1d` | PASS |
| Gitee `main` and feature branch contain `d7edc1d` | PASS |
| Python / Node runtime | PASS: Python 3.11.15 / Node 24.14.0 |
| Contract tests | PASS: 261 |
| Crawler-lite tests | PASS: 6 |
| Acceptance tests | PASS: 110 |
...
| Real MySQL integration | PASS: 40 passed, 2 explicitly skipped |
| Alembic offline SQL and real MySQL head | PASS: `20260809_0046`, 111 AIRank tables |
...
The wider default regression separately passed `602 passed, 40 skipped`. Skips are explicit environment-dependent gates and are not counted as production evidence.
...
Unknown external publication outcomes now have a governed local reconciliation path instead of a force-success or replay button. Alembic `0046` adds tenant/project-scoped cases, immutable event hashes and an attempt back-reference. Only WordPress/HTTP packages with matching `outcome_unknown` attempt and `external_started` Operation Guard may enter the flow. Submission requires the real URL, actual 2xx status, external receipt ID, timezone-aware observation time and a same-project immutable object whose stored bytes, size and SHA-256 are rechecked. A second authenticated delivery admin must echo the selected object ID/hash and independently approve or reject. Rejection leaves all ledgers unknown and non-replayable; approval performs no external call and atomically closes the Guard, attempt, package/lineage and case. Every manual receipt remains `receipt_origin=manual_reconciliation`, `two_person_manual_evidence` and `external_delivery_verified=false`; publish/update only return to `delivered`, not `published`. The console now requires the actual HTTP status rather than inserting a fixed 200 and uses the correct local observation time.

API, Worker and Scheduler were restarted after migration with process-private four-Provider configuration and the verified Python 3.11 dependency path. Runtime health returns HTTP 200, the reconciliation detail route returns the expected `PUBLISH_RECONCILIATION_NOT_FOUND` contract for a nonexistent case, MySQL reports `20260809_0046` and 111 `airank_*` tables, and all four route manifests remain enabled on the verified models. After explicitly fixing runtime mode to `api`, the restarted endpoint completed one current L3 generation probe per route: Doubao, Qianwen, Kimi and DeepSeek all returned `ready`, `l3_generation` and `generation_verified=true`. Two earlier Browser-L2 responses from the misconfigured restart were discarded and are not evidence. The current API startup section has no error markers, Worker remains alive and Scheduler records `tick_completed`. This smoke check did not call a customer publishing endpoint, accept Consumer Browser as generation evidence or create a fake success receipt.
...
- Operation Guard now protects Provider credential writes, initial external publishing, post-publication update/withdraw actions and the two-person local resolution of proved-success unknown outcomes. Route control, price-version writes and other high-risk admin mutations still require staged migration; generic HTTP results without sufficient proof remain unknown and are intentionally not auto-retried or force-resolved.
...
- Consumer Browser L3 was not run or accepted from this engineering gate. Two accidental L2 readiness checks caused by a restart-mode fallback produced no generation evidence and were discarded; explicit L3 execution authorization stayed disabled, so the latest valid Consumer result remains 0/4. Login/human-verification blockers cannot be replaced by Provider API success or L2 page interaction.
...
- No production customer WordPress/HTTP initial/update/withdraw receipt, real two-person reconciliation case, reviewer benchmark, or elapsed T+7/T+14/T+30 outcome evidence is available yet.

## Decision

The governed publication update/withdraw and unknown-outcome reconciliation slices are engineering-complete and dual-remote synchronized at the verified commit, while Provider Vault, Skill Trust, Usage Ledger, model lifecycle and Publisher Operation Guard remain intact. AIRank is not authorized to claim production or commercial readiness. Obtain customer test-site credentials and receipts, a real independent reconciliation case, v4-pro quota and target-model approval, production auth/storage, then run the explicitly authorized external gates.
