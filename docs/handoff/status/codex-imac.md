...
| M2-IMAC-001B | review_env_blocked | this commit | MySQL-backed worker lease store now reads/writes `airank_async_jobs`; real local MySQL validation is still blocked by access denied for the dev `airank` user. |
