...
| M2-IMAC-001B | review | this commit | MySQL-backed worker lease store reads/writes `airank_async_jobs`; CodexMacPro verified the real local MySQL path on 2026-05-17. |
...
- 2026-05-17: `M2-IMAC-001B` production hardening added `MySQLJobLeaseStore` for `airank_async_jobs`. It preserves the same claim/heartbeat/succeed/fail/timeout/retry transitions as the in-memory store while persisting state, payload, result, structured errors, lock owner, and heartbeat fields. Validation: `cd apps/worker && python3 -m pytest -q` passed 10 tests; contracts 56, acceptance 9, score 3, evidence 9, xinghe-adapter 2, and web build passed; `git diff --check` passed. Initial real DB validation was environment-blocked at that time and is superseded by the CodexMacPro fresh-DB verification below.
- 2026-05-17: CodexMacPro release verification cleared the `M2-IMAC-001B` MySQL environment blocker on a fresh `airank_laike_release_gate` database: `MySQLJobLeaseStore` claim, heartbeat, succeed, timeout sweep, and explicit retry all passed. A timezone normalization fix was added so MySQL naive datetimes are treated as UTC before comparing with worker `now`.
