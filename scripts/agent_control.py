...
def parse_open_tasks(owner: str) -> list[dict[str, str]]:
    ...
    for line in read(LAUNCH_BOARD).splitlines():
        ...
        if task_owner != owner or normalized_status not in OPEN_TASK_STATUSES:
            continue
        tasks.append({"task": task, "status": status, "exit_criteria": exit_criteria})
    return tasks
...
def build_next_prompt(agent_key: str) -> str:
    ...
    task_lines = "\n".join(
        f"- [{task['status']}] {task['task']} :: exit={task['exit_criteria']}" for task in tasks[:8]
    )
    if not task_lines:
...
This prompt is generated by `scripts/agent_control.py`. Do not paste an old prompt from chat. Treat this file plus `{agent["role_prompt"].relative_to(ROOT)}` as the current instruction source.
