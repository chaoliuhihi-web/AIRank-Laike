...
AGENTS = {
    "codex-win": {
        ...
        "status_file": STATUS_DIR / "codex-win.md",
    ...
    "codex-imac": {
        ...
        "status_file": STATUS_DIR / "codex-imac.md",
    ...
    "codex-macpro": {
        ...
        "status_file": STATUS_DIR / "codex-macpro.md",
...
def parse_status_overrides() -> dict[str, str]:
    overrides: dict[str, str] = {}
    for status_file in sorted(STATUS_DIR.glob("*.md")):
        for line in read(status_file).splitlines():
            ...
            if len(parts) < 2 or parts[0] == "Packet":
                continue
            packet_match = PACKET_ID_RE.search(parts[0])
            if not packet_match:
                continue
            overrides[packet_match.group(0)] = parts[1]
    return overrides
...
def parse_tasks_from_packets(owner: str) -> list[dict[str, str]]:
    ...
    status_overrides = parse_status_overrides()
    for task in all_tasks:
        if task["packet_id"] in status_overrides:
            task["status"] = status_overrides[task["packet_id"]]
...
def build_next_prompt(agent_key: str) -> str:
    ...
    owner = agent["owner"]
    status_file = agent["status_file"].relative_to(ROOT)
...
This prompt is generated by `scripts/agent_control.py`. Do not paste an old prompt from chat. Treat this file, `docs/handoff/execution-packets.md`, `{status_file}`, and `{agent["role_prompt"].relative_to(ROOT)}` as the current instruction source.
...
If a command is not applicable because that package is not initialized, write the reason in `{status_file}`.

## Status Update Rule

Update `{status_file}` for your packet status and run log. Do not edit `docs/handoff/execution-packets.md` or `docs/handoff/review-ledger.md` unless CodexMacPro explicitly asks you to resolve a planning/review conflict.
