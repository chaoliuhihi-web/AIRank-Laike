...
import sys
import time
from apps.api.provider_scan import browser_provider_config, launch_provider_context, probe_provider_readiness
from playwright.sync_api import sync_playwright

provider = os.environ["PROVIDER"]
config = browser_provider_config(provider)
with sync_playwright() as playwright:
    context = launch_provider_context(playwright, config, headless=False)
    ...
    if sys.stdin.isatty() and os.environ.get("AIRANK_PROVIDER_LOGIN_CONFIRM", "enter") != "close":
        input("[provider-login] press Enter here after the prompt box is visible...")
    else:
        print("[provider-login] close the opened browser window after the prompt box is visible...")
